...
# -*- coding: utf-8 -*-
"""
PasswordRuleMiner v2 — Password Artifact Generator
...
v2 improvements:
- Phrase rules (multi-char injections) are clearly separated from
  transform rules (single-char / structural operations).
  Use --no-phrase-rules to generate only transform rules.
- Separate output files: _transform.rule and _phrase.rule
- Reorganized code sections for easier navigation and maintenance
"""
import argparse
import hashlib
import logging
...
# =============================================
# RULE CLASSIFICATION: TRANSFORM vs PHRASE
# =============================================
def is_phrase_rule(rule: str) -> bool:
    """
    Return True if this rule injects a multi-character chunk (>= 2 chars).

    Phrase rules inject words, username fragments, mined prefix/suffix strings
    of length >= 2, or numeric sequences. They are the rules that "look like
    words/phrases" when decoded, e.g. ``^e ^d ^o ^c`` (prepend "code") or
    ``$2 $0 $2 $4`` (append "2024").

    Transform rules use single-char or structural operations only:
    case (l/u/c/C/t/E), toggle (TN), rotate ({/}), swap (*NM),
    bitwise shift (L/R), delete (DN), or single-char leet (sXY).
    """
    if not rule:
        return False
    ops = rule.split()
    prepend_count = sum(1 for op in ops if len(op) == 2 and op[0] == "^")
    append_count = sum(1 for op in ops if len(op) == 2 and op[0] == "$")
    return prepend_count >= 2 or append_count >= 2
...
class PasswordRuleMiner:
    def __init__(self, output_dir: Path, use_cache: bool = True, max_workers: int = None,
                 ...
                 rule_format: str = "hashcat", include_phrase_rules: bool = True):
        self.out = output_dir
        ...
        # -------------------------------------------------------
        # Phrase vs Transform rule separation (v2 addition)
        # -------------------------------------------------------
        # include_phrase_rules=False skips all multi-char injection
        # rules (username fragments, custom words, prefix/suffix >= 2,
        # year sequences) so only structural transforms are generated.
        self.include_phrase_rules: bool = include_phrase_rules
        # Separate scored lists filled during generation; used in write_rules
        # to produce _transform.rule and _phrase.rule output files.
        self.transform_rules: List[Tuple[int, str]] = []
        self.phrase_rules: List[Tuple[int, str]] = []
    ...
    def _add_scored_rules(self, rules: List[Tuple[int, str]]):
        """Thread-safe method to add scored rules and classify into transform/phrase buckets.

        Phrase rules are silently dropped when include_phrase_rules=False.
        """
        with self._rules_lock:
            for score, rule in rules:
                if is_phrase_rule(rule):
                    if not self.include_phrase_rules:
                        continue
                    self.phrase_rules.append((score, rule))
                else:
                    self.transform_rules.append((score, rule))
    ...
    def _append_rule(self, score: int, rule: str):
        """Add a single (score, rule) pair and classify it into phrase/transform bucket.
    ...
    def generate_custom_wordlist_rules(self):
        """Generate phrase rules based on custom wordlists (multi-char injections)."""
        if not self.include_phrase_rules:
            if self.verbose:
                log.debug("Phrase rules disabled — skipping custom wordlist rules")
            return
        ...
        for word in list(self.custom_words)[:500]:  # Limit for performance
            if len(word) <= 6:
                ...
                if prep:
                    self._append_rule(SCORE_CUSTOM_WORDLIST, prep)
                if app:
                    self._append_rule(SCORE_CUSTOM_WORDLIST, app)
    ...
    # -------------------------------------------------------
    # Rule generation
    # Phrase-only methods check self.include_phrase_rules.
    # -------------------------------------------------------
    def generate_user_context_rules(self):
        """Generate phrase rules from username fragments (multi-char injections).

        Username parts of 3-10 chars produce prepend/append rules that inject
        multi-character chunks and are classified as phrase rules.
        Skipped entirely when include_phrase_rules=False.
        """
        if not self.include_phrase_rules:
            log.info("Phrase rules disabled — skipping per-username context rules")
            return
        ...
        for username in self.usernames:
            ...
            for part in parts:
                ...
                if self.rule_format in ["hashcat", "both"]:
                    ...
                    if prep_low:
                        self._append_rule(SCORE_USER_CONTEXT_HIGH, prep_low)
                    if prep_cap:
                        self._append_rule(SCORE_USER_CONTEXT_MEDIUM, prep_cap)
                    if app_low:
                        self._append_rule(SCORE_USER_CONTEXT_MEDIUM, app_low)
                    if app_cap:
                        self._append_rule(SCORE_USER_CONTEXT_LOW, app_cap)
    ...
    def generate_unified_base_list(self):
        ...
        """
        log.info("Generating unified base list (00_combined_bases.txt)...")
        
        # Read all base files if they exist
        base_files = [
        ...
        for base_file in base_files:
            ...
            with base_file.open("r", encoding="utf-8") as f:
                for line in f:
                    base = line.strip()
                    if base:  # Skip empty lines
                        unified_bases.add(base)
        
        # Check if we have any unified bases
        if not unified_bases:
            log.warning("No unified bases were found to generate the combined file.")
            return
        
        # Sort the unified list alphabetically or by frequency if available
        sorted_bases = sorted(unified_bases)
        
        # Debugging information
        log.debug(f"Debug: Unified bases sorting, combined_bases length: {len(unified_bases)}")
        log.debug(f"Debug: Unified bases (first 10): {list(unified_bases)[:10]}")

        # Write to a new file
        combined_file = self.out / "00_combined_bases.txt"
        with combined_file.open("w", encoding="utf-8") as f:
            f.write("\n".join(sorted_bases) + "\n")
        
        log.info(f"→ Unified base list generated: {combined_file.name} ({len(sorted_bases):,} entries)")
        ...
        for base, rule_counter in sorted(base_to_rules.items(), key=lambda x: sum(x[1].values()), reverse=True):
            ...
            for rule, count in rule_counter.most_common(3):  # Top 3 rules per base
                if count >= 2:  # Only include if seen at least twice
                    # Skip phrase rules when phrase generation is disabled
                    if not self.include_phrase_rules and is_phrase_rule(rule):
                        continue
                    # Score based on frequency and base popularity
                    score = count * 100000 + identified_bases[base] * 1000
                    self._append_rule(score, rule)
                    rule_count += 1
    ...
    def generate_prefix_suffix_rules(self):
        """Generate phrase rules from mined potfile prefix/suffix strings (length >= 2).

        All rules produced here inject >= 2 chars and are phrase rules.
        Skipped entirely when include_phrase_rules=False.
        """
        if not self.include_phrase_rules:
            log.info("Phrase rules disabled — skipping prefix/suffix rules")
            return
    ...
    def generate_surround_rules(self):
        """Generate phrase rules that surround a word with prefix+suffix (both >= 2 chars).

        Skipped entirely when include_phrase_rules=False.
        """
        if not self.include_phrase_rules:
            log.info("Phrase rules disabled — skipping surround rules")
            return
        ...
        for prefix, pc in self.prefix.most_common(500):
            ...
            for suffix, sc in self.suffix.most_common(500):
                ...
                if rule not in seen:
                    seen.add(rule)
                    self._append_rule(int((pc + sc) * 18), rule)

    def generate_static_and_year_rules(self):
        """Generate static high-value rules and year/season phrase rules.

        Static rules like ``l c $2 $0 $2 $4 $!`` inject multi-char numeric
        sequences, so they are treated as phrase rules and skipped when
        include_phrase_rules=False.
        """
        if not self.include_phrase_rules:
            log.info("Phrase rules disabled — skipping static and year rules")
            return
    ...
    def generate_bfs_complex_rules(self):
        ...
        Pure transform rules (BFS structural ops, swaps, numeric single-char
        substitutions) are always generated.  Phrase-style combo rules that
        inject mined prefix/suffix strings are only generated when
        include_phrase_rules=True.
        """
        log.info("Generating BFS-based complex rules with advanced operations...")

        # --- Transform rules (always generated) ---
        # Basic BFS structural operations
        bfs_rules = self.bfs_generator.generate()
        self._add_scored_rules(bfs_rules)
        log.info(f"Generated {len(bfs_rules):,} BFS exploration rules")

        # Rotation and swap combinations
        rotation_swap_rules = self.bfs_generator.generate_rotation_swap_combos()
        self._add_scored_rules(rotation_swap_rules)
        ...
        for swap_rule in swap_rules:
            self._append_rule(SCORE_ROTATION_SWAP_LOW, swap_rule)
        log.info(f"Generated {len(swap_rules):,} advanced swap rules")

        # Numeric sequence rules (single-char substitutions + short appends)
        numeric_rules = generate_numeric_sequence_rules()
        for num_rule in numeric_rules:
            self._append_rule(SCORE_NUMERIC, num_rule)
        log.info(f"Generated {len(numeric_rules):,} numeric sequence rules")

        # --- Phrase rules (mined string injections, skipped if disabled) ---
        if not self.include_phrase_rules:
            log.info("Phrase rules disabled — skipping BFS combination/combined prepend-append rules")
            return

        # BFS combinations with common mined strings (phrase-style)
        ...
        self._add_scored_rules(combo_rules)
        log.info(f"Generated {len(combo_rules):,} BFS combination rules")

        # Combined prepend/append rules from mined prefixes and suffixes
        ...
        for comb_rule in combined_rules:
            self._append_rule(SCORE_COMBINED_RULES, comb_rule)
    ...
    def write_rules(self):
        """Write all rule output files.

        Combined files (01_elite / 02_extended_50k / 03_complete) contain all
        generated rules sorted by score.

        v2 additions — separate typed files (always written when format includes hashcat):
          • ``_transform.rule`` — structural/single-char operations only
          • ``_phrase.rule``    — multi-char injection rules (username fragments,
                                  mined prefix/suffix strings ≥ 2, year sequences,
                                  custom words)  — empty if --no-phrase-rules was used
        """
        log.info("Writing rule files...")

        def _dedupe_sorted(scored_list: List[Tuple[int, str]]) -> List[str]:
            scored_list.sort(key=lambda x: x[0], reverse=True)
            seen: Set[str] = set()
            out = []
            for _, rule in scored_list:
        ...
        if self.rule_format in ["hashcat", "both"]:
            ...
            # Combined (all rules)
            unique_rules = _dedupe_sorted(self.scored_rules)
            ...
            # Transform-only rules (structural / single-char ops)
            transform_unique = _dedupe_sorted(self.transform_rules)
            write_file(self.out / "03_complete_transform.rule", transform_unique)

            # Phrase rules (multi-char injection rules)
            phrase_unique = _dedupe_sorted(self.phrase_rules)
            write_file(self.out / "03_complete_phrase.rule", phrase_unique)

            log.info(
                f"Rule split: {len(transform_unique):,} transform rules, "
                f"{len(phrase_unique):,} phrase rules"
            )
        ...
        if self.rule_format in ["john", "both"]:
            ...
            seen_jtr: Set[str] = set()
            unique_jtr_rules = [
                r for _, r in self.jtr_scored_rules
                if r not in seen_jtr and not seen_jtr.add(r)
            ]
...
def main():
    parser = argparse.ArgumentParser(
        description="PasswordRuleMiner v2 — Artifact Generator with Advanced Features"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable phrase (multi-char injection) rule generation. "
            "Only structural/transform rules will be produced "
            "(case, toggle, rotate, swap, single-char leet). "
            "Skips: username fragment rules, mined prefix/suffix rules (len>=2), "
            "surround rules, year/static rules, and custom-wordlist injection rules."
        ),
    )
    ...
    include_phrase_rules = not args.no_phrase_rules
    if not include_phrase_rules:
        log.info("Phrase rule generation DISABLED (--no-phrase-rules) — transform rules only")
    else:
        log.info("Phrase rule generation enabled (use --no-phrase-rules to disable)")
    ...
    miner = PasswordRuleMiner(
        ...
        include_phrase_rules=include_phrase_rules,
    )
